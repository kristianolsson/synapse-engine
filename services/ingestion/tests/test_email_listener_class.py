...
from services.ingestion.email_listener import EmailListener, config
...
class TestEmailListenerClass(unittest.TestCase):
    ...
    @patch("services.ingestion.email_listener.IMAPClient")
    ...
    @patch("services.ingestion.email_listener.email.message_from_bytes")
    ...
    def test_run_loop_basics(self, mock_imap_cls):
        # This test is tricky because run() is an infinite loop.
        # We'll rely on side_effect to break the loop by setting _running = False
