...
- **Pass `stats=` through, don't pre-format or gate it yourself.**
  `send_reply(..., stats=some_dict_or_None)` (`channels/email/reply.py`) and
  `send_telegram_message(..., stats=some_dict_or_None)`
  (`channels/telegram/sender.py`) both format and gate the stats footer
  internally — always pass the raw dict through. Exception:
  `channels/telegram/listener.py` and `core/scheduler.py`'s inline-reply path
  still predate the `stats=` kwarg and gate/format manually
  (`if session_manager.get_stats_enabled(key): text +=
  format_stats_telegram(stats)`); don't copy that into new code.
